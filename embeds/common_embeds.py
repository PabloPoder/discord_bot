...
def create_avatar_embed(user: Member):
    ...
    joined_at = user.joined_at.strftime(
        "%b %d, %Y") if user.joined_at else " _ "
...
def create_weather_embed(weather_data: WeatherData):
    ...
    embed.add_field(name='Feels Like',
                    value=weather_data.feels_like, inline=True)
    embed.add_field(name='Sky Condition',
                    value=weather_data.description, inline=True)
    ...
    embed.add_field(
        name='Clouds', value=f"{weather_data.clouds} %", inline=True)
...
def create_commands_embed(image: Asset):
    ...
    embed.add_field(name="`/avatar`",
                    value="Displays the avatar of an user.", inline=False)
    embed.add_field(name="`/eeorigins`",
                    value="Gives you the guide to make Origins EE", inline=False)
    embed.add_field(
        name="`/weather`",
