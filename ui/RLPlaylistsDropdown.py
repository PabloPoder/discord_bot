...
class RLPlaylistsDropdown(Select):
  def __init__(self, playlists: List[Playlist] = None, player: RocketLeaguePlayer = None):
    ...
    self.player = player
    options = [SelectOption(label=playlist.name, value=playlist.name) for playlist in self.playlists]

    super().__init__(options=options, placeholder="Select a playlist to view its details...")

  async def callback(self, interaction: Interaction):
    # Get the selected book
    playlist = next(playlist for playlist in self.playlists if playlist.name == interaction.data["values"][0])
