...
async def fetch_player_data(nametag: str):
    ...
    try:
        ...
        logger.error(
            f"An error occurred while fetching the player data: {err}")
...
async def get_rocket_league_stats_data(nametag: str):
    ...
    playlists_data = [Playlist.from_dict(playlist)
    ...
    player = RocketLeaguePlayer.from_data(
        data, playlists_data, lifetime_playlist)
